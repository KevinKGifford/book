...
Draw negative numbers in red and positive numbers in green.  <span style="color:red">RED</span>
...
{% for number in numbers %}
    ...
    <rect x="{{loop.index * 30}}" width="20" height="{{magnitude}}" style="fill:rgb(0,0,255);stroke-width:3;stroke:rgb(0,0,0)" />
{% endfor %}
</svg>
...
{% for number in numbers %}
    {% set magnitude = number %}
    <rect y="{{loop.index * 30}}" width="{{magnitude}}" height="20" style="fill:rgb(0,0,255);stroke-width:3;stroke:rgb(0,0,0)" />
{% endfor %}
</svg>
...
<table>
    {% for rows in data %}
        <tr>
            <!-- Add your code here to index into array  -->
...
{% for point in data %}
    <circle cx="{{point[0]*10}}" cy="{{point[1]*2}}" r="2" stroke="black" stroke-width="3" fill="red" />
{% endfor %}
</svg>
...
<svg width="500" height="200" style="border:1px solid grey">
{% for point in data %}
    <circle cx="{{point[0]*10}}" cy="{{point[1]*2}}" r="{{point[2]}}" stroke="black" stroke-width="3" fill="red" />
{% endfor %}
</svg>
